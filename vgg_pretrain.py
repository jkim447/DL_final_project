...
class Ensemble(torch.nn.Module):
  ...
  def __init__(self, orig_resnet, orig_vgg16):
    super().__init__()
    #self.orig_resnet = orig_resnet
    #self.final_linear = torch.nn.Linear(512, 200) # TODO
    self.orig_vgg16 = orig_vgg16
    self.vgg16_final_linear = torch.nn.Linear(4096, 200)
    #self.ensemble_final_linear = torch.nn.Linear(400, 200)

  def forward(self, x):
    # Resnet
   # x1 = self.orig_resnet.conv1(x)
   # x1 = self.orig_resnet.bn1(x1)
   # x1 = self.orig_resnet.relu(x1)
   # x1 = self.orig_resnet.maxpool(x1)

   # x1 = self.orig_resnet.layer1(x1)
   # x1 = self.orig_resnet.layer2(x1)
   # x1 = self.orig_resnet.layer3(x1)
   # x1 = self.orig_resnet.layer4(x1)

   # x1 = self.orig_resnet.avgpool(x1)
   # x1 = x1.view(x1.size(0), -1)
   # # REQUIRES GRAD
   # x1 = self.final_linear(x1)
  
    # VGG net in parallel
    x2 = self.orig_vgg16.features(x)
    x2 = x2.view(x2.size(0), -1)
    x2 = self.orig_vgg16.classifier[0](x2)
    x2 = self.orig_vgg16.classifier[1](x2)
    x2 = self.orig_vgg16.classifier[2](x2)
    x2 = self.orig_vgg16.classifier[3](x2)
    x2 = self.orig_vgg16.classifier[4](x2)
    x2 = self.orig_vgg16.classifier[5](x2)
    # REQUIRES GRAD
    x2 = self.vgg16_final_linear(x2) 
   ...
   # x = torch.cat((x1,x2), dim=1)
    # REQUIRES GRAD
   # x_= self.ensemble_final_linear(x)
    x = F.log_softmax(x2, dim = 1)
    return x
...
def run_experiment(args):
    ...
    # create an ensemble of resnet and VGG
    model = Ensemble(None, vgg16)
    print(model)
    model.cuda()
    # Choose optimizer
    # change it back to model.parameters() if training from scratch
    #optimizer = optim.Adam([{'params':model.final_linear.parameters()}, 
    #    {'params':model.vgg16_final_linear.parameters()}, 
    #    {'params':model.ensemble_final_linear.parameters()}], lr = 0.001)
...
run_experiment(Args(batch_size = 32, test_batch_size = 50))
